...
const Cart = ({ setOpenCart }) => {
  ...
  const uniqueShopIds = cart
    ? [...new Set(cart.map((item) => item.shopId))]
    : [];
  const hasMultipleSellers = uniqueShopIds.length > 1;
        ...
        {cart && cart.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto bg-white">
              ...
              {hasMultipleSellers && (
                <div className="bg-yellow-50 border-l-4 border-yellow-400 p-3 mx-4 mt-3 mb-2">
                  <div className="flex">
                    ...
                    <div className="ml-3">
                      <p className="text-sm text-yellow-700">
                        <strong>Note:</strong> You can only checkout items from
                        ONE seller at a time. Please remove items from other
                        sellers to proceed.
                      </p>
                      <p className="text-xs text-yellow-600 mt-1">
                        {uniqueShopIds.length} sellers in cart
                      </p>
                    </div>
                  </div>
                </div>
