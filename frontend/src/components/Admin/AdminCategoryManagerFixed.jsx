...
const AdminCategoryManager = () => {
  ...
  // Stable function to load categories
  ...
  }, [mounted, loadCategoriesData, dispatch]);

  // Effect for data changes (debounced)
  useEffect(() => {
    if (mounted) {
  ...
  // Handle tree view
  useEffect(() => {
    if (view === "tree" && mounted) {
      dispatch(getCategoryTree());
    }
  }, [view, dispatch, mounted]);
  ...
  // Handle success messages (no reload to prevent loop)
  useEffect(() => {
    if (message) {
          ...
          <div className="flex items-center flex-1">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-500">
                Level {category.level} • {category.productCount} products
              </p>
            </div>
  ...
  if (!mounted || (isLoading && !categories.length)) {
    return <Loader />;
  }
